class Board {
  ...
  makeMove(move) {
    ...
    if (
      ...
      Math.abs(move.startSq - move.endSq) == 16
    ) {
    ...
    if (move.endSq == null) {
      this.hash = xor(
        this.hash,
        zobristTable[move.startSq][
          this.pos[move.startSq].type - 1 + this.col * 6
        ]
      ); // removing piece without end pos
    ...
    if (this.pos[move.endSq] != null) {
      this.hash = xor(
        this.hash,
        zobristTable[move.endSq][this.pos[move.endSq].type - 1 + this.col * 6]
      ); // removing capture from hash
    }
    this.hash = xor(
      this.hash,
      zobristTable[move.startSq][this.pos[move.startSq].type - 1 + this.col * 6]
    ); // removing piece from hash
  ...
  unmakeMove(move) {
    ...
    if (move.attack != null) {
      this.hash = xor(
        this.hash,
        zobristTable[move.attack.pos][move.attack.type - 1 + 6 * (1 - this.col)]
      ); // adding captured piece
    }
    ...
    for (let cur of fen.split(" ")[0]) {
      ...
      if (cur.match("^[0-9]+$")) {
        ...
        switch (cur.toLowerCase()) {
          case "p":
            this.pos[pos] = new Piece(
              pos,
              1,
              cur.toLowerCase() === cur ? 0 : 1
            );
