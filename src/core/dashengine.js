    ...
    if (i % 8 == 0 && i != 0) {
      ...
      if (board.pos[y * 8 + x] == null) {
        ...
        if (board.pos[y * 8 + x].col == 0) {
          output +=
            "|\x1b[40m " +
            getLookup(board.pos[y * 8 + x].type, 0) +
            " \x1b[0m| " +
            (y + 1);
        } else {
          output +=
            "|\x1b[47m\x1b[30m " +
            getLookup(board.pos[y * 8 + x].type, 1) +
            " \x1b[0m| " +
            (y + 1);
        }
      }
      ...
      if (board.pos[y * 8 + x] == null) {
        ...
        if (board.pos[y * 8 + x].col == 0) {
          output +=
            "|\x1b[40m " + getLookup(board.pos[y * 8 + x].type, 0) + " \x1b[0m";
        } else {
          output +=
            "|\x1b[47m\x1b[30m " +
            getLookup(board.pos[y * 8 + x].type, 1) +
...
var board = new Board();
let testFen = "7K/1k6/8/8/4Q3/2b5/2p5/8 w - - 0 1";
board.setup();
