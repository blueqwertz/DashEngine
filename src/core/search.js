async function Deepening(time, view = false) {
  ...
  for (let tempDepth = 1; true; tempDepth++) {
    ...
    let [curBestMove, curScore] = Search(
      tempDepth,
      true,
    ...
    if (view) {
      console.log(
        `${tempDepth} ply: ${totalNodes}, time left: ${
          time - (Date.now() - start)
        }`
      );
    }
...
function IterativeDeepening(depth) {
  ...
  for (let tempDepth = 1; tempDepth < depth + 1; tempDepth++) {
    ...
    console.log(
      `${tempDepth} ply: ${Date.now() - start} ms, nodes: ${totalNodes}`
    );
  ...
  console.log(
    (Date.now() - begin).toString() + "ms, nodes",
    iterativeNodes.toString()
  );
  tt.clear();
  return bestMove;
}
...
function Search(depth = 1, searchScore = false, timeRemaining = 999999) {
  ...
  console.log(
    (Date.now() - start).toString() + " ms, nodes:",
    totalNodes.toString()
  );
  ...
  function orderMoves(moves) {
    ...
    for (let move of moves) {
      ...
      if (move.attack != null) {
        scoreGuess =
          10 * getPieceScore(move.attack) - getPieceScore(startPiece);
      }
...
function Evaluate() {
  ...
  whiteEval += forceKingToEdge(
    whiteMaterial,
    blackMaterial,
    blackEndGameWeight,
    true
  );
  blackEval += forceKingToEdge(
    blackMaterial,
    whiteMaterial,
    whiteEndGameWeight,
    false
  ...
  function forceKingToEdge(myMat, opponentMat, endgameWeight, isWhite) {
    ...
    if (myMat > opponentMat + scores[0] * 2 && endgameWeight > 0) {
      ...
      let opponenKingDstToCenterFile = Math.max(
        3 - yourKingFile,
        yourKingFile - 4
      );
      let opponenKingDstToCenterRank = Math.max(
        3 - yourKingRank,
        yourKingRank - 4
      );
      let opponenKingDstToCenter =
        opponenKingDstToCenterFile + opponenKingDstToCenterRank;
