...
function addSetting(name, value) {
  ...
  if (typeof value == "number") {
    settingsDiv.innerHTML += `<div class="container">
                                    <span>${getBetterName(
                                      name
                                    )}</span><input type="number" id="${name}" value="${value}" max="100000" oninput="settings['${name}'] = parseFloat(this.value);
                                    document.getElementById('allSettingsString').innerHTML = JSON.stringify(settings)">
                                </div>`;
  }
}
...
function setSettings() {
  ...
  document.getElementById("allSettingsString").innerHTML = JSON.stringify(
    settings
  );
  ...
  for (let i = 63; i > -1; i--) {
    ...
    if ((x + y) % 2 == 0) {
      document.getElementById(
        "boardbg"
      ).innerHTML += `<div class="file light" style="grid-collumn:${x}; grid-row:${
        7 - y
      }"></div>`;
    } else {
      document.getElementById(
        "boardbg"
      ).innerHTML += `<div class="file dark" style="grid-collumn:${x}; grid-row:${
        7 - y
      }"></div>`;
    }
...
function addPiece(piece, x, y) {
  ...
  divboard.innerHTML += `<div class="piece ${code}" id="${
    x + y * 8
  }" style="transform: translate(${x * 100}%, ${(7 - y) * 100}%)"></div>`;
}
...
function dragElement(elmnt) {
  ...
  function closeDragElement(e) {
    var dist = Math.sqrt(
      Math.abs(startX - e.clientX) ** 2 + Math.abs(startY - e.clientY) ** 2
    );
    if (dist > 20) {
      lookMove(file, rank);
    }
...
function drag(ind) {
  ...
  if (board.pos[ind].col == board.col) {
    divoverlay.innerHTML += `<div class="selected" style="grid-column: ${
      (ind % 8) + 1
    }; grid-row: ${7 - Math.floor(ind / 8) + 1}"></div>`;
  }
  if (cur.col == board.col) {
    ...
    for (let i = 0; i < curmoves.length; i++) {
      ...
      divoverlay.innerHTML += `<div class="content" id="p${
        move.endSq
      }" onclick="makedisplaymove(${i})" style="grid-column: ${
        (move.endSq % 8) + 1
      }; grid-row: ${
        7 - Math.floor(move.endSq / 8) + 1
      }"><div class="point"></div></div>`;
      if (board.pos[move.endSq] != null || move.enPassant != null) {
        document
          .getElementById(`p${move.endSq}`)
          .children[0].classList.add("attack");
      }
    }
  }
}
...
async function makedisplaymove(ind, show = false) {
      ...
      for (let i = 0; i < 4; i++) {
        document
          ...
          .children[i].addEventListener("click", function () {
            document
              .getElementById("select")
              .classList.remove("display-normal");
            resolve(i);
          });
      }
    });
  ...
  if (!show) {
    if (movesMade % 2 == 0) {
      document.getElementById("moves").innerHTML += `<div class="row" id="m${
        movesMade / 2
      }"></div>`;
    }
    document.getElementById("m" + Math.floor(movesMade / 2)).innerHTML +=
      "<div class='move'>" + convertToPgn(move) + "</div>";
  }
  ...
  if (move.promotionType == null) {
    ...
    board.moves.push(
      posToLetter(move.startSq) +
        posToLetter(move.endSq) +
        getLookup(move.promotionType, 0)
    );
  }
  ...
  if (move.castle != null) {
    ...
    document.getElementById(move.castle[0]).style = `transform: translate(${
      x * 100
    }%, ${(7 - y) * 100}%)`;
  ...
  lastMovesDiv.innerHTML += `<div class="content lastmoveend" style="transform: translate(${
    x * 100
  }%, ${(7 - y) * 100}%)"></div>`;
  lastMovesDiv.innerHTML += `<div class="content lastmovestart" style="transform: translate(${
    ((move.startSq % 8) - 1) * 100
  }%, ${(7 - Math.floor(move.startSq / 8)) * 100}%)"></div>`;
  document.getElementById(move.startSq).style = `transform: translate(${
    x * 100
  }%, ${(7 - y) * 100}%)`;
...
function unmakedisplaymove() {
  ...
  lastMovesDiv.innerHTML += `<div class="content lastmovestart" style="transform: translate(${
    ((move.endSq % 8) - 1) * 100
  }%, ${(7 - Math.floor(move.endSq / 8)) * 100}%)"></div>`;

  document.getElementById(move.endSq).style = `transform: translate(${
    x * 100
  }%, ${(7 - y) * 100}%)`;
  ...
  if (move.castle != null) {
    ...
    document.getElementById(move.castle[1]).style = `transform: translate(${
      x * 100
    }%, ${(7 - y) * 100}%)`;
...
function closeSideBar(el) {
  if (!el.parentElement.parentElement.classList.toString().includes("hide")) {
    el.parentElement.parentElement.children[1].style.maxHeight =
      el.parentElement.parentElement.children[1].offsetHeight + "px";
  } else {
    el.parentElement.parentElement.children[1].style.maxHeight = null;
  }
  settings["ui"][
    el.parentElement.parentElement.id
  ] = el.parentElement.parentElement.classList.toString().includes("hide");
