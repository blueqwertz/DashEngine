  ...
  generateAttacksAndPins() {
    ...
    for (let i = 0; i < len; i++) {
      ...
      if (
        ...
        if (checked.length == 1) {
          if (
            board.pos[move.startSq].type == 2 ||
            board.pos[move.startSq].type == 4 ||
            board.pos[move.startSq].type == 5
          ) {
            let rowdif =
              Math.floor(move.endSq / 8) - Math.floor(move.startSq / 8);
    ...
    for (let dir = 0; dir < 8; dir++) {
      ...
      for (let n = 0; n < edges[dir]; n++) {
        ...
        if (board.pos[target] != null) {
          if (
            enPassantPos != null &&
            board.pos[target].col != board.col &&
            !enPassantNotPos &&
            (board.pos[target].type == index[dir] ||
              board.pos[target].type == 5) &&
            !(offsets[dir] == coloff)
          ) {
          ...
          if (board.pos[target].col != board.col && !pinnotpos) {
            if (
              (pospin && board.pos[target].type == index[dir]) ||
              board.pos[target].type == 5
            ) {
      ...
      if (!onlyAttacks) {
        ...
        if (!checked || blockcheck[p.pos + offsets[1]]) {
          if (pinned[p.pos] == 1 || pinned[p.pos] == 0) {
            if (
              edges[1] > 1 &&
              board.pos[p.pos + offsets[1]] == null &&
              board.pos[p.pos + offsets[0]] == null &&
              !p.moved
            ) {
              if (promotion) {
          ...
          if (!blocked[p.pos + i - 8]) {
            if (!checked || blockcheck[p.pos + i - 8]) {
              ...
              if (pinned[p.pos] == 0 || pinned[p.pos] == 6 + -i) {
                if (
                  board.pos[p.pos + i] != null &&
                  board.enPassant == p.pos + i &&
                  edges[edgeoff] > 0
                ) {
                  moves.push(
                    new Move(p.pos, p.pos - 8 + i, board.pos[p.pos + i])
                  );
                }
              }
            }
